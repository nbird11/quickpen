...
export const progressService = {
  ...
  getProgress: async (
    ...
    try {
      ...
      let sprintsQuery = db.collection('sprints')
        .where('userId', '==', userId)
      ...
      const allSprints = snapshot.docs.map(doc => {
        ...
        return {
          id: doc.id,
          ...data,
          completedAt: (data.completedAt as firebase.firestore.Timestamp).toDate()
        } as Sprint;
      });
...
function calculateCurrentStreak(sprints: Sprint[], timezone: string): number {
  ...
  for (let i = 1; i < sortedSprints.length; i++) {
    ...
    const expectedPrevDate = getDateStr(new Date(lastDateObj.setDate(lastDateObj.getDate() - 1)));
